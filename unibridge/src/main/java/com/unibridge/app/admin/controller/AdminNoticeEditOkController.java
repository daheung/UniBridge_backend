		...
		while ((part = parser.readNextPart()) != null) {
			...
			if (part.isParam()) { // 현재 part가 일반 텍스트 데이터 인지 확인
				...
				if ("boardNumber".equals(paramName)) {
					...
					boardDTO.setNoticeboardNumber(boardNumber); // dto에 게시글 번호 저장
				...
				// 기존 파일 삭제
				if (boardNumber != 0) {
					FileDTO existingFiles = fileDAO.selectFile(boardDAO.fileNumberSelect(boardNumber)); // 해당 게시글에 연결된 기존 첨부파일 목록을 DB에서 조회
					File oldFile = new File(UPLOAD_PATH, existingFiles.getFileName()); // 실제 서버 파일 객체 생성
						if (oldFile.exists()) { // 해당 파일이 실제로 존재하는지 확인
							System.out.println("기존 파일 삭제 : " + oldFile.getAbsolutePath());
							oldFile.delete(); // 파일 삭제
						}
					}
					fileDAO.delete(boardDAO.fileNumberSelect(boardNumber)); // 파일 DB에서도 해당 정보 삭제
				...
				if (fileOriginalName != null) { // 사용자가 실제 새로운 파일을 선택했을 때만 실행
					...
					boardDTO.setFileNumber(fileDAO.insertFileIfExists(fileDTO));
